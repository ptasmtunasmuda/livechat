    ...
    <div class="flex flex-col h-full">
        ...
        <div class="bg-white border-b border-gray-200 px-6 py-4">
            <div class="flex items-center justify-between">
                ...
                <div class="flex items-center space-x-2">
                    ...
                    <!-- Room menu -->
                    <button @click="showRoomMenu = !showRoomMenu" class="p-2 text-gray-500 hover:text-gray-700 hover:bg-gray-100 rounded-lg">
        ...
        <div class="flex-1 flex">
            ...
            <div class="flex-1 flex flex-col">
                ...
                <!-- Message Input -->
                <MessageInput v-if="room && room.id" :room-id="room.id" @message-sent="onMessageSent" />
            </div>

            <!-- User List Sidebar (for larger screens) -->
            <div class="hidden lg:block w-64 bg-gray-50 border-l border-gray-200">
                <UserList :users="roomMembers" :current-user="currentUser || {}" :show-presence="true" />
            </div>
        </div>
    </div>
</template>
...
// Computed properties
const roomMembers = computed(() => props.room?.members?.map((member) => member.user).filter(Boolean) || []);

const onlineCount = computed(() => roomMembers.value.filter((user) => user?.is_online).length);
