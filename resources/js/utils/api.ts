...
export const chatApi = {
  ...
  getRoom: (roomId: number): Promise<ApiResponse<ChatRoom>> =>
    api.get(`/rooms/${roomId}`),
    
  createRoom: (roomData: CreateRoomForm): Promise<ApiResponse<ChatRoom>> =>
    api.post('/rooms', roomData),
    
  updateRoom: (roomId: number, roomData: Partial<ChatRoom>): Promise<ApiResponse<ChatRoom>> =>
    api.put(`/rooms/${roomId}`, roomData),
    
  deleteRoom: (roomId: number): Promise<ApiResponse> =>
    api.delete(`/rooms/${roomId}`),
    
  joinRoom: (roomId: number): Promise<ApiResponse<ChatRoom>> =>
    api.post(`/rooms/${roomId}/join`),
    
  leaveRoom: (roomId: number): Promise<ApiResponse> =>
    api.post(`/rooms/${roomId}/leave`),
    
  // Messages
  getMessages: (roomId: number, page = 1): Promise<ApiResponse<PaginatedResponse<Message>>> =>
  ...
  editMessage: (messageId: number, data: { content: string }): Promise<ApiResponse<Message>> =>
    api.put(`/messages/${messageId}`, data),
    
  deleteMessage: (messageId: number): Promise<ApiResponse> =>
    api.delete(`/messages/${messageId}`),
    
  markRoomAsRead: (roomId: number): Promise<ApiResponse> =>
...
export const presenceApi = {
  updateStatus: (status: 'online' | 'away' | 'busy' | 'offline'): Promise<ApiResponse> =>
    api.post('/user/presence', { status }),
